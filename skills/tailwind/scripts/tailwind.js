...
const { generateTailwindConfig } = require('../../../lib/power-engine');
const { c, header } = require('../../../lib/output');

const HELP = `dragoon tailwind - generate one tailwind.config.js combining all detected tokens

USAGE
  dragoon tailwind [--apply] [--manifest p] [--out path] [--json]

OUTPUT
  one consolidated tailwind config: colors + spacing + fontFamily + fontSize +
...
FLAGS
  ...
  --out <path>    output path (default: tailwind.config.js)
...
function loadManifest(root, manifestPath) {
  if (manifestPath) { try { return JSON.parse(fs.readFileSync(manifestPath, 'utf8')); } catch (_e) { return null; } }
  const conv = path.join(root, 'dragoon.json');
  if (fs.existsSync(conv)) { try { return JSON.parse(fs.readFileSync(conv, 'utf8')); } catch (_e) { return null; } }
...
function main() {
  ...
  if (!manifest) { console.error('tailwind: no manifest. run dragoon scan first.'); return 1; }

  let content;
  try { content = generateTailwindConfig(manifest); }
  catch (e) { console.error(`tailwind: ${e.message}`); return 1; }
  const relPath = args.out || 'tailwind.config.js';
  ...
  process.stdout.write(header('tailwind'));
